...
void CRealtimezoomscrollDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_GammaValue3, m_ValueG);
	DDX_Control(pDX, IDC_BetaValue2, m_ValueF);
	DDX_Control(pDX, IDC_GammaValue2, m_ValueE);
	DDX_Control(pDX, IDC_AlphaValue2, m_ValueD);
...
BEGIN_MESSAGE_MAP(CRealtimezoomscrollDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &CRealtimezoomscrollDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CRealtimezoomscrollDlg::OnBnClickedButton2)
	ON_WM_SYSCOMMAND()
END_MESSAGE_MAP()
...
BOOL CRealtimezoomscrollDlg::OnInitDialog()
{
	...
	SetWindowText(_T("Pressure measurement indicator"));
	...
	for (int i = 0; i < sampleSize; ++i)
		m_timeStamps[i] = m_dataSeriesA[i] = m_dataSeriesB[i] = m_dataSeriesC[i] = m_dataSeriesD[i] = m_dataSeriesE[i] = m_dataSeriesF[i] = m_dataSeriesG[i] = Chart::NoValue;
	m_currentIndex = 0;
...
void CRealtimezoomscrollDlg::OnDataRateTimer()
{
	...
	do
	{
		...
		m_dataSeriesD[m_currentIndex] = dataD;
		m_dataSeriesE[m_currentIndex] = dataE;
		m_dataSeriesF[m_currentIndex] = dataF;
		m_dataSeriesG[m_currentIndex] = dataG;
		++m_currentIndex;
	...
	sprintf(buffer, " %.2f", m_dataSeriesD[m_currentIndex - 1]);
	m_ValueD.SetWindowText(CString(buffer));

	sprintf(buffer, " %.2f", m_dataSeriesE[m_currentIndex - 1]);
	m_ValueE.SetWindowText(CString(buffer));

	sprintf(buffer, " %.2f", m_dataSeriesF[m_currentIndex - 1]);
	m_ValueF.SetWindowText(CString(buffer));
	
	sprintf(buffer, " %.2f", m_dataSeriesG[m_currentIndex - 1]);
	m_ValueG.SetWindowText(CString(buffer));
...
void CRealtimezoomscrollDlg::drawChart(CChartViewer* viewer)
{
	...
	// Create an XYChart object of size 850 x 549 pixels
	XYChart* c = new XYChart(850, 549);
	...
	c->addTitle("    Pressure change over time", "arial.ttf", 18);
	...
	c->yAxis()->setTitle("Pressure", "arialbd.ttf", 12);
	...
	layer->addDataSet(viewPortDataSeriesA, 0xff0000, "Sensor1");
	layer->addDataSet(viewPortDataSeriesB, 0x00cc00, "Sensor2");
	layer->addDataSet(viewPortDataSeriesC, 0x0000ff, "Sensor3");
	layer->addDataSet(viewPortDataSeriesD, 0x008000, "Sensor4");
	layer->addDataSet(viewPortDataSeriesE, 0xbfff00, "Sensor5");
	layer->addDataSet(viewPortDataSeriesF, 0xeee6c4, "Sensor6");
	layer->addDataSet(viewPortDataSeriesG, 0x005666, "Sensor7");
...
UINT CRealtimezoomscrollDlg::ThreadFunction(LPVOID _mothod)
{
	CRealtimezoomscrollDlg* pDlg = (CRealtimezoomscrollDlg*)_mothod;
...
// Start button (receive thread create)
void CRealtimezoomscrollDlg::OnBnClickedButton1()
{
...
// thread pause
void CRealtimezoomscrollDlg::OnBnClickedButton2()
{
	if (m_pThread == NULL)
	{
...
// For control thread when closing dialog
void CRealtimezoomscrollDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	if (nID == SC_CLOSE)
	{
		m_pThread->SuspendThread();
